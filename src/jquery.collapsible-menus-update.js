		...
		$('#' + sId + ' li ul').parent('li')
			...
			.each(function() {
				...
				$(this).on('click', function() {
					...
					var $menu = $(this).next();
					var $menuHTML = $(this).children('a').html();
					//Added Jcooper 12/28/2012
					//Collapse all other open menus
					$('#' + sId + ' li ul').parent('li').each( function() {
						var $_menuHTML = $(this).prev('li').children('a').html();
						if($_menuHTML != $menuHTML) { // this is not the one clicked, so close it... if open
